...
class Jeye{
  constructor(source, options){
    Object.assign(this, {
      source: path__default.normalize(source),
      options,
      targets: {},
      dependents: {},
      subscribers: {}
    ...
    totalist(this.source, (rel) => {
      let p = path__default.join(this.source, rel);
      if(!this.isHidden(p)){
  ...
  async updateDependents(p){
    if(path__default.extname(p) !== '.js') return;
    await init;
    let code = fs__default.readFileSync(p, 'utf8');
    ...
    imps = imps.forEach(({ s, e }) => {
      ...
      if(import_str.startsWith('.')){
        ...
        let cwdimport = path__default.join(p, '../', import_str);
