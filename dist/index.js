...
const toStats = util.promisify(fs.stat);
const toRead = util.promisify(fs.readdir);
			...
			arr.map(str => {
				...
				return toStats(abs).then(stats => {
					return stats.isDirectory()
						? totalist(abs, callback, path.join(pre, str))
						: callback(path.join(pre, str), abs, stats)
				});
			})
		);
	});
...
async function targets(sources=[], options={}){
  ...
  sources.map(async src => {
    await totalist(src,  (rel) => {
      path__default.push(path__default.join(sources[i], rel));
    });
  });
  await Promise.all(promises); 
