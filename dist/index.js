...
let isHiddenFile = (p) => path__default.basename(p).startsWith('.') || path__default.basename(p).startsWith('_');

function watch(toWatch, callback=()=>{}){
  toWatch = path__default.normalize(toWatch);
  let initialFiles = [];
  let watcher;
  let dependents = {};
  ...
  async function updateDependents(p){
    await init;
    let source = fs__default.readFileSync(p, 'utf8');
    ...
    imps = imps.forEach(({ s, e }) => {
      ...
      if(rawimport.startsWith('.')){
        ...
        rawimport = path__default.extname(rawimport) ? rawimport : rawimport + '.js';
        // convert the import path to be relative to the cwd
        let cwdimport = path__default.join(p, '../', rawimport);
  ...
  totalist(toWatch, (rel) => {
    if(!isHiddenFile(rel)){
      initialFiles.push(path__default.join(toWatch, rel));
    }
  });
